...
class Device:
    ...
    * Use :meth:`set_default_instance` / :meth:`get_default_instance` to
    ...
    def set_default_instance(cls, instance):
        ...
        :param instance: A BuildHAT instance, or ``None`` to clear.
    ...
    @classmethod
    def _key_for(cls, bhat):
        """Return the registry key for a BuildHAT instance."""
        for key, inst in cls._registry.items():
            if inst is bhat:
                return key
        return id(bhat)
    ...
    def _setup(cls, device=SERIAL_DEV, reset_gpio=RESET_GPIO_NUMBER,
               boot0_gpio=BOOT0_GPIO_NUMBER, debug=False, hat_instance=None):
        """Resolve (or create) a BuildHAT instance.

        Resolution order
        ~~~~~~~~~~~~~~~~
        1. ``hat_instance`` supplied → return it directly.
        2. Canonical device path already in registry → return existing HAT.
        3. Create a new HAT, register it, apply promotion rule:

           - No default yet → becomes default.
           - Explicit ``device`` path given → always becomes default
             (last-explicit-init wins; enables ``setUpClass`` pattern).
           - ``device=None`` → only promotes when no default exists yet.
        """
        ...
        bhat = cls._create_buildhat(canonical, reset_gpio, boot0_gpio, debug)
    ...
    def __init__(self, port, device=SERIAL_DEV,
        ...
        :param device: Serial device path, or ``None`` for the default HAT.
        :param reset_gpio: Reset GPIO number.
        :param boot0_gpio: Boot0 GPIO number.
        :param debug: Enable debug logging.
        :param hat_instance: Bind to a specific BuildHAT instance instead of
        ...
        # Per-HAT port guard: (registry_key, port_index)
        self._used_key = (Device._key_for(self._hat_instance), p)
        if Device._used.get(self._used_key, False):
        ...
        Device._used[self._used_key] = True

    def __del__(self):
        if hasattr(self, "_used_key") and Device._used.get(self._used_key, False):
            Device._used[self._used_key] = False
    ...
    def isconnected(self):
        """Assert device is present and of the expected type.
